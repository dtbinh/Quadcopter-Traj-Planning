...
% close the reading from the document
fclose(fid);
...
for i = 1: blockNum
    ...
    upperR = bsxfun(@min, bsxfun(@max,ceil(bsxfun(@rdivide, ...
                         bsxfun(@minus, map.block{i}.upperright + margin, map.boundary.lowerleft), ...
                         res)), 1), unitL);
    lowerL = bsxfun(@min, bsxfun(@max,ceil(bsxfun(@rdivide, ...
                         bsxfun(@minus, map.block{i}.lowerleft - margin, map.boundary.lowerleft), ...
                         res)), 1), unitL);
    mapdata(lowerL(1):upperR(1), lowerL(2):upperR(2), lowerL(3):upperR(3)) = 1;
end

map.ary2re = @ (c) bsxfun(@plus, bsxfun(@times, c - .5, grid), map.boundary.lowerleft);
